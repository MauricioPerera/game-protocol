...
| `bounds` | entry[] | no | each: `{ rule, level?, collection\|singleton, field \| arrayField(+itemField), gt?, min?, max?, integer?, required?, msg?() }` — range family. `arrayField` (collection only) reaches fields **inside a nested array** (e.g. `waves.*.spawns[].count`), using the same convention as `refs` |
| `matches` | entry[] | no | each: `{ rule, level?, collection\|singleton, field \| arrayField(+itemField), pattern, required?, msg?() }` — text-property family: the string value must match `pattern` (a RegExp source, **not auto-anchored** — use `^…$` for a full match). A missing or non-string value is skipped (that is `bounds`/`required`'s job) unless `required` is set. A `pattern` that does not compile is rejected at profile load, not mid-lint |
...
| `grids` | entry[] | no | each: `{ rule, emptyRule?, level?, collection\|singleton, rowsField?, legendField?, fillField?, shape?, legend?: { rule, level?, tileField?, tileTarget: { collection, allow? }, palField?, palMax? } }` — grid/legend family (§11). Without `shape`, validates uniform row width (`rule`) and empty rows (`emptyRule`, defaults to `rule`) self-consistently (against the grid's own first row). With `shape`, validates row/col count and does not treat empty rows as an error on its own; `shape` comes in two forms: **by reference** `{ singleton, rowsField?, colsField? }`, when the shape lives in another token (e.g. `platform.rows`/`.cols` for `monster-rpg`/`tower-defense` maps), or **literal** `{ rows?, cols? }` (positive integers), when the shape is intrinsic to the genre and lives in no token (e.g. a peg-solitaire board is 7×7 because that is the game). Each check is independent: a missing target field is not enforced. Either way, each legend/fill symbol's `tile` (and optional `pal`) is checked against a target collection |
...
- **Pure-data rules — fourth stage shipped.** The `enums` family, optional `refs[].msg`, JSON profile loading, the `grids` family (§6.1) and the `arrayField`/`itemField` aggregate form of `refs[].target` already made *simple* profiles fully data-only (`profiles/quiz.json` is the reference; `adventure`/`dungeon` moved their scene-grid checks and `monster-rpg`/`tower-defense` moved their map-grid checks to `grids`; `dungeon`'s `warp-lock` cross-scene aggregate moved to `refs`). The fourth stage closes the two gaps the pure-data profiles had declared in their own `$comment`:
  - **`matches`** (§6.1) — a text property as a RegExp, so string shape is declarable: `sudoku`'s 81-character `grid`/`solution` and `peg-solitaire`'s 7-character board rows are now validated as data instead of only by the reference simulation.
  - **`bounds` over `arrayField`** — fields *inside* a nested array, which `bounds` could not reach: `shooter`'s `waves.*.spawns[].count/gap` are now covered.

  Both families are borrowed from the sibling project [KDD](https://github.com/MauricioPerera/KDD), whose rule-contract engine had already generalised this protocol's `refs`/`bounds`/`enums` to business rules and grown `matches`/`each` there first; the `matches` semantics (not auto-anchored, non-strings skipped) are deliberately identical so a rule reads the same in both.

  What remains is genuinely bespoke per-genre logic (balance smells like `type-symmetry`/`wave-monotonic`, multi-field structural checks like `path-contiguous`, and cross-field consistency like a sudoku's `grid` agreeing with its `solution`) that does not reduce to a closed set of declarative shapes without becoming a general-purpose query language — the point past which `rules` functions are the right tool, not a gap to close.
