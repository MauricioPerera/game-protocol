...
(function (factory) {
  ...
  function ruleTileArt({ data, add }) {
    ...
    for (const [id, mat] of Object.entries(data.tileArt || {})) {
      if (!(id in tiles)) add('warn', 'tileart-ref', 'tileArt define el tile ' + id + ' no declarado en `tiles`');
      if (!Array.isArray(mat) || mat.length !== 8 || mat.some(r => !Array.isArray(r) || r.length !== 8))
        ...
        add('error', 'tileart-dims', 'tileArt ' + id + ' tiene un indice de color fuera de 0..' + (palCount - 1));
    }
  }
