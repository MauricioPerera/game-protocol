...
// solid-sync (cross-check OPCIONAL con el motor): activar con GAME_ENGINE=<ruta al archivo del motor>.
// Cruza tiles.solid con el Set de IDs solidos del codigo (regex `<NOMBRE>_SOLID_TILES = new Set([...])`).
// Si GAME_ENGINE no esta definido, el cruce se omite en silencio (este repo es portable, sin motor).
let solidSet = null;
const enginePath = process.env.GAME_ENGINE;
if (enginePath) {
  try {
    const sim = fs.readFileSync(path.resolve(root, enginePath), 'utf8');
    const m = sim.match(/SOLID_TILES\s*=\s*new Set\([^[]*\[([^\]]+)\]/);
    if (m) solidSet = new Set(m[1].split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n)));
    else add('warn', 'solid-sync', 'GAME_ENGINE no contiene un `*_SOLID_TILES = new Set([...])`');
  } catch (e) { add('warn', 'solid-sync', 'GAME_ENGINE no se pudo leer: ' + enginePath); }
}
if (solidSet) {
  for (const [id, t] of Object.entries(tiles)) {
    const n = Number(id), inSet = solidSet.has(n);
    if (t.solid === true && !inSet) add('error', 'solid-sync', 'tile ' + id + ' (' + t.name + ') es solid en GAME.md pero NO esta en el Set del motor');
    if (t.solid === false && inSet) add('error', 'solid-sync', 'tile ' + id + ' (' + t.name + ') es walkable en GAME.md pero SI esta en el Set del motor');
  }
...
// dead-token (cross-check OPCIONAL con el motor): activar con GAME_ENGINE.
// Avisa de claves de `balance` declaradas en GAME.md pero nunca referenciadas en el codigo del motor.
// Cubre el patron gBal('x'), acceso .x y ['x'] (evita el drift "knob declarado pero ignorado").
if (enginePath) {
  try {
    const sim = fs.readFileSync(path.resolve(root, enginePath), 'utf8');
  ...
  } catch (e) { /* GAME_ENGINE no legible: cross-check omitido */ }
}
